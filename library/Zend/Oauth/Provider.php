 ...
 * @author smalyshev/Zend_OAuth_Provider
 */
class Zend_Oauth_Provider
{
	 ...
	 * @throws Zend_Oauth_Exception
	 */
	public function checkOAuthRequest(Zend_Uri_Http $url = null, $params = array(), $debug = false)
	{
	    if(empty($url)) {
        ...
        //if($debug)
        {
         /*   \Zend_Debug::dump($params, 'paaarams');
            \Zend_Debug::dump($this->token_secret, 'token secret from provider');
            \Zend_Debug::dump($this->token, 'token from prov');
            \Zend_Debug::dump($this->consumer_secret, 'cons secret from prov');
            \Zend_Debug::dump($method, 'method from prov'); */
        }
	...
	public function generateToken($size)
	{
	    ...
	    while(strlen($str) < $size) {
	        $str .= md5(uniqid(mt_rand(), true), false);
	    }
